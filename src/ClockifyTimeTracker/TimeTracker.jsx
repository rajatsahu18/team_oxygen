...
import {FaRegCalendarAlt, FaTrashAlt, FaPlay} from "react-icons/fa"
    ...
    const handleaddproject = () => {
      ...
      const handlestartdescriptionagain = (e) => {
        ...
        const handledeletedescription = (e) => {
            const { projecttitle, descriptiontitle } = JSON.parse(e.target.value);
            axios
              .get(`https://json-server-projects.herokuapp.com/userdata/${id}`)
              .then((res) => {
                ...
                axios
                  .patch(
                    `https://json-server-projects.herokuapp.com/userdata/${id}`,
                    {
                      projects: updateddes
                    }
                  )
                ...
                {projects && projects.map((le) => {
                    ...
                    return (
                        <div className = {styles.projectDataDetails}>
                            ...
                            <button className = {styles.timerButton} value={JSON.stringify(dataobjex)} onClick={handlestartdescriptionagain} > <FaPlay size = "18px" /> </button>
                            <button className = {styles.deleteButton} value={JSON.stringify(dataobjex)} onClick={handledeletedescription}> <FaTrashAlt size = "18px" /> </button>
                        </div>
                    );
                    });
                })}
