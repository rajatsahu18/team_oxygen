...
export const getdatauser = (payload) => (dispatch) => {
  ...
  axios
    .get(
      `https://json-server-mocker-pooran.herokuapp.com/userdata/${payload.id}`
    )
    .then((res) => {
...
export const descriptionaddfunction = (query) => (dispatch) => {
  axios
    .get(`https://json-server-mocker-pooran.herokuapp.com/userdata/${query.id}`)
    .then((res) => {
      ...
      if (thirdobj.length === obj[0].description.length) {
        ...
        axios
          .patch(
            `https://json-server-mocker-pooran.herokuapp.com/userdata/${query.id}`,
            {
              projects: des
            }
          )
...
export const updatedescriptionfunction = (payload)=>(dispatch)=>{
  axios.get(`https://json-server-mocker-pooran.herokuapp.com/userdata/${payload.id}`)
  .then((res) => {
